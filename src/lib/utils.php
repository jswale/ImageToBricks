<?PHP
include_once realpath(dirname(__FILE__)) . "/logger.php";
...
function ciede2000 ($c1, $c2) {
	 ...
	 * Observations"
	 * by Gaurav Sharma, Wencheng Wu and Edul N.
	 * Dalal.
	 */
	...
	$G = 0.5 *
			(1 -
			sqrt(pow($a_C1_C2, 7.0) / (pow($a_C1_C2, 7.0) + pow(25.0, 7.0)))); // (4)
	$a1p = (1.0 + $G) * $a1; // (5)
	$a2p = (1.0 + $G) * $a2; // (5)
	...
	$T = 1 - 0.17 * cos(radians($a_hp - 30)) + 0.24 * cos(radians(2 * $a_hp)) +
			0.32 * cos(radians(3 * $a_hp + 6)) - 0.20 * cos(
					radians(4 * $a_hp - 63)); // (15)
	...
	$dE = sqrt(
			pow($dLp / ($SL * $kL), 2) + pow($dCp / ($SC * $kC), 2) +
					pow($dHp / ($SH * $kH), 2) +
...
function a_hp_f ($C1, $C2, $h1p, $h2p) { // (14)
	...
	else if ((abs($h1p - $h2p) > 180) && (($h1p + $h2p) < 360)) return ($h1p +
			$h2p + 360) / 2.0;
	else if ((abs($h1p - $h2p) > 180) && (($h1p + $h2p) >= 360)) return ($h1p +
			$h2p - 360) / 2.0;
